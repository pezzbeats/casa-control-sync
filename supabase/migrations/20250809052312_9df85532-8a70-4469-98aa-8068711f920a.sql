-- Create home automation schema (initial setup)
-- 1) Tables
...
create policy "Public can read sensors" on public.sensor_events for select using (true);
...
create policy "Anyone can update devices state (demo)" on public.devices for update using (true) with check (true);

-- Optional: allow inserting sensor events (e.g., webhooks) for demo
create policy "Anyone can insert sensor events (demo)" on public.sensor_events for insert with check (true);
